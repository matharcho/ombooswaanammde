...
local function ensureESPForRock(rockInfo)
	...
	label.Text = "â›ï¸ " .. tostring(rockInfo.rockType)
...
local function mobileMelt(minigameGui)
    setStatus("Mobile: Melt minigame")
    ...
    -- Smoothly fill bar
    for i = 0, 1, 0.05 do
        if not minigameGui.Visible or not autoForge.enabled then
            return false
        end

        bar.Area.Size = UDim2.fromScale(1, i)
        task.wait(0.05)
    end

    bar.Area.Size = UDim2.fromScale(1, 1)
    task.wait(0.5)
...
local function mobilePour(minigameGui)
    setStatus("Mobile: Pour minigame")
    ...
    while minigameGui.Visible and autoForge.enabled do
        local targetY = area.Position.Y.Scale + (area.Size.Y.Scale / 2)
        line.Position = UDim2.fromScale(line.Position.X.Scale, targetY)

        if timer.Bar.Size.X.Scale >= 0.98 then
            break
        end
...
local function waitForMoldBreak(timeout)
    local start = tick()
...
local function mobileHammer(minigameGui)
    setStatus("Mobile: Hammer minigame")

    if not waitForMoldBreak(10) then
        setStatus("Mold break timeout")
        return false
    end
    ...
    while minigameGui.Visible and autoForge.enabled do
        for _, note in ipairs(minigameGui:GetChildren()) do
            if note:IsA("GuiObject") and note.Visible and not clicked[note] then
                local frame = note:FindFirstChild("Frame")
                if frame then
                    local circle = frame:FindFirstChild("Circle")
                    local border = frame:FindFirstChild("Border")

                    if circle and border then
                        local diff = math.abs(circle.Size.Y.Scale - border.Size.Y.Scale)
                        if diff <= 0.05 then
                            -- Directly activate instead of clicking
                            if note:IsA("ImageButton") or note:IsA("TextButton") then
                                note:Activate()
                            end
                            clicked[note] = true
                        end
...
local function autoPlayForge()
    while forgeGui and autoForge.enabled do
        local mode, gui = getCurrentMinigame(forgeGui)

        if mode == "Melt" then
            mobileMelt(gui)
        elseif mode == "Pour" then
            mobilePour(gui)
        elseif mode == "Hammer" then
            mobileHammer(gui)
        end
